...
def calculate_ce_loss(y, txt, vid_len, txt_len, sil_index, pau_index, a_index, i_index,u_index, e_index,o_index,penalty_weight=100.0):
    """
    ...
    penalty_weight: 無音クラスに加えるペナルティ重み
    """
    # log_softmaxを使って対数確率を計算
    log_probs = F.log_softmax(y, dim=-1)  # (batch_size, seq_len, num_classes)

    # クロスエントロピー損失を計算（reduction='none'で各フレームごとに損失を計算）
    ce_loss = F.nll_loss(log_probs.view(-1, log_probs.size(-1)), txt.view(-1), ignore_index=-1, reduction='none')

    # 無音クラスにペナルティを加える
    penalties = torch.ones_like(txt, dtype=torch.float)  # デフォルトでペナルティなし
    penalties[txt == sil_index] = penalty_weight  # 'sil'クラスにペナルティを加える
    penalties[txt == pau_index] = penalty_weight  # 'pau'クラスにペナルティを加える
    penalties[txt == a_index] = penalty_weight
    penalties[txt == i_index] = penalty_weight
    penalties[txt == u_index] = penalty_weight
    penalties[txt == e_index] = penalty_weight
    penalties[txt == o_index] = penalty_weight

    # ペナルティを適用した損失を計算
    weighted_loss = ce_loss * penalties.view(-1)
...
def test(model, net):
    ...
    with torch.no_grad():
        dataset = MyDataset(opt.video_path,
            ...
            'test')
        # 無音クラスのインデックスを取得
        sil_index = dataset.sil_index  # silクラスのインデックス
        pau_index = dataset.pau_index  # pauクラスのインデックス
        a_index = dataset.a_index
        i_index = dataset.i_index
        u_index = dataset.u_index
        e_index = dataset.e_index
        o_index = dataset.o_index
        ...
        for (i_iter, input) in enumerate(loader):            
            ...
            # クロスエントロピー損失を計算
            loss = calculate_ce_loss(y, txt, vid_len, txt_len,sil_index,pau_index, a_index, i_index,u_index, e_index,o_index).detach().cpu().numpy()
...
def train(model, net):
    ...
    for epoch in range(opt.max_epoch):
        for (i_iter, input) in enumerate(loader):
            ...
            optimizer.zero_grad()
            y = net(vid)
            loss = calculate_ce_loss(y, txt, vid_len, txt_len,sil_index,pau_index, a_index, i_index,u_index, e_index,o_index)
            #loss = calculate_ce_loss(y, txt, vid_len, txt_len, class_weights).detach().cpu().numpy()
