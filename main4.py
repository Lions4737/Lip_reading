...
def test(model, net):
    ...
    with torch.no_grad():
        ...
        for (i_iter, input) in enumerate(loader):            
            ...
            loss = calculate_ce_loss(y, txt, vid_len, txt_len, class_weights)
        ...
        return (np.array([loss.cpu().numpy() for loss in loss_list]).mean(), np.array(wer).mean(), np.array(cer).mean())
...
if __name__ == '__main__':
    ...
    # 音素ごとの重みを計算
    frequencies = torch.tensor(list(phoneme_frequencies.values()), dtype=torch.float32)

    # 重みを計算（頻度が低いほど重みを大きくする）
    class_weights = frequencies  # 頻度の逆数
    class_weights = class_weights / class_weights.sum()  # 重みを正規化

    # 確認のため、計算された重みを表示
    print("Calculated Class Weights:")
    for phoneme, weight in zip(phoneme_frequencies.keys(), class_weights):
        print(f"{phoneme}: {weight.item():.6f}")
