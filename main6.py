...
# 以下一部を例として変更
# クロスエントロピー損失を計算する関数
'''def calculate_ce_loss(y, txt, vid_len, txt_len, valid_classes=None):
    """
    クロスエントロピー損失を計算し、無効なクラスを無視します。
    ...
    valid_classes: 有効なクラスのリストまたはセット
    """
    # log_softmaxを使って対数確率を計算
    log_probs = F.log_softmax(y, dim=-1)  # (batch_size, seq_len, num_classes)
    
    # 有効クラスのみを使用するためのマスキング
    if valid_classes is not None:
        mask = torch.zeros(log_probs.size(-1), dtype=torch.bool).cuda()
        mask[list(valid_classes)] = True  # 有効クラスに対応するインデックスをTrueに設定
        log_probs = log_probs[:, :, mask]  # 有効なクラスのみ抽出

    # クロスエントロピー損失を計算
    # ターゲットも有効クラスの範囲内に収める
    if valid_classes is not None:
        remap_dict = {cls: i for i, cls in enumerate(sorted(valid_classes))}
        txt = txt.clone()  # テンソルをクローンして変更可能にする
        for orig_cls, new_cls in remap_dict.items():
            txt[txt == orig_cls] = new_cls

    # ignore_index = -1 により無効なターゲットクラスを無視
...
def calculate_ce_loss(y, txt, vid_len, txt_len):
    """
    クロスエントロピー損失を計算し、`sil`と`pau`を無視します。
    y: モデルの出力 (batch_size, seq_len, num_classes)
    txt: ターゲット音素列 (batch_size, seq_len)
    vid_len: 予測フレームの長さ (batch_size)
    txt_len: 音素列の長さ (batch_size)
    """
    # サイレンスとポーズのインデックスを取得
    sil_index = MyDataset.sil_index
    pau_index = MyDataset.pau_index

    # log_softmaxを使って対数確率を計算
    log_probs = F.log_softmax(y, dim=-1)  # (batch_size, seq_len, num_classes)

    # ターゲットラベルで sil と pau を -1 に置き換える
    txt = txt.clone()  # 元のテンソルを変更しないようクローン
    txt[txt == sil_index] = -1
    txt[txt == pau_index] = -1

    # ignore_index = -1 により sil と pau を無視して損失計算
    loss = F.nll_loss(log_probs.view(-1, log_probs.size(-1)), txt.view(-1), ignore_index=-1)
    return loss
